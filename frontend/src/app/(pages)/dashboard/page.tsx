    ...
    return (
        <div className="space-y-5 jakarta">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-[11px] tracking-widest text-[#0A5482] uppercase font-medium mb-1">Admin panel</p>
                    <h1 className="gloock text-2xl text-[#0d1f2d]">Good morning, Admin ✦</h1>
                    <p className="text-sm text-slate-400 mt-1">Nepal Summit Trials · Dashboard overview</p>
                </div>
                <div className="flex items-center gap-2.5">
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-500">
                        <TbCalendarCheck className="text-sm" /> {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {stats.map(({ label, value, delta, up, icon: Icon, accent, bg, ic, badge }) => (
                    <div key={label} className="bg-white rounded-2xl p-5 border border-slate-100 relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${bg} ${ic}`}>{badge}</span>
                        </div>
                        <div className="jakarta text-[26px] font-semibold text-[#0d1f2d]">{value}</div>
                        <div className="text-xs text-slate-400 font-medium mt-0.5">{label}</div>
                        <div className={`text-[11px] mt-2 flex items-center gap-1 font-medium ${up === true ? 'text-emerald-600' : 'text-slate-400'}`}>
                            {up === true ? <TbTrendingUp className="text-xs" /> : <TbClock className="text-xs" />}
                            {delta}
                        </div>
                    </div>
                ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                ...
                <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h2 className="lora text-[15px] text-[#0d1f2d]">Recent bookings</h2>
                            <p className="text-[11px] text-slate-400 mt-0.5">Last 5 transactions</p>
                        </div>
                        <Link href="/dashboard/bookings" className="flex items-center gap-1 text-xs font-medium text-[#0A5482] hover:underline">
                            View all <TbArrowRight className="text-xs" />
                        </Link>
                    </div>
                    {bookings.map((b) => (
                        <div key={b.name} className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0">
                            <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-lg ${b.ib} flex items-center justify-center text-[10px] font-bold flex-shrink-0`}>
                                    {b.initials}
                                </div>
                                <div>
                                    <p className="text-[13px] font-semibold text-slate-800">{b.name}</p>
                                    <p className="text-[11px] text-slate-400">{b.trek}</p>
                                </div>
                            </div>
                            <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${badgeStyle[b.status]}`}>{b.status}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="lg:col-span-2 flex flex-col gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5">
                        <h2 className="lora text-[15px] text-[#0d1f2d] mb-1">Popular treks</h2>
                        <p className="text-[11px] text-slate-400 mb-4">By booking volume</p>
                        {treks.map(({ name, pct }) => (
                            <div key={name} className="flex items-center gap-3 mb-3 last:mb-0">
                                <span className="text-xs text-slate-400 font-medium w-24 shrink-0 truncate">{name}</span>
                                <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full bg-[#0A5482]" style={{ width: `${pct}%`, opacity: pct > 60 ? 1 : pct > 40 ? 0.7 : 0.45 }} />
                                </div>
                                <span className="text-[11px] font-semibold text-slate-400 w-7 text-right">{pct}%</span>
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Quick actions */}
                    <div className="bg-white rounded-2xl border border-slate-100 p-5">
                        <h2 className="lora text-[15px] text-[#0d1f2d] mb-3">Quick actions</h2>
                        <div className="grid grid-cols-2 gap-2">
                            {quickActions.map(({ label, icon: Icon, href, bg, ic }) => (
                                <Link key={label} href={href} className="flex items-center gap-2 px-3 py-2.5 border border-slate-100 rounded-xl hover:bg-slate-50 transition text-[12px] font-medium text-slate-600 bg-slate-50/50">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h2 className="lora text-[15px] text-[#0d1f2d]">Revenue this year</h2>
                            <p className="text-[11px] text-slate-400 mt-0.5">Monthly breakdown</p>
                        </div>
                        <span className="text-[11px] font-semibold text-emerald-700 bg-green-50 px-2.5 py-0.5 rounded-full">↑ 8%</span>
                    </div>
                    {/* Replace with your chart component */}
                    <div className="h-20 flex items-end gap-1.5">
                        {[28, 35, 42, 38, 55, 62, 48, 70, 65, 58, 74, 80].map((v, i) => (
                            <div key={i} className="flex-1 rounded-t-sm" style={{ height: `${(v / 80) * 100}%`, background: i === 5 ? '#0A5482' : '#b5d4f4' }} />
                        ))}
                    </div>
                    <div className="flex gap-1.5 mt-1.5">
                        {['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D'].map((m, index) => (
                            <div key={`${m}-${index}`} className="flex-1 text-center text-[9px] text-slate-300">
                                {m}
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h2 className="lora text-[15px] text-[#0d1f2d]">Recent activity</h2>
                            <p className="text-[11px] text-slate-400 mt-0.5">Latest updates</p>
                        </div>
                        <button className="text-xs font-medium text-[#0A5482] hover:underline flex items-center gap-1">
                            See all <TbArrowRight className="text-xs" />
                        </button>
                    </div>
                    {activity.map(({ text, time, bg, ic, icon: Icon }) => (
                        <div key={text} className="flex items-start gap-3 py-2.5 border-b border-slate-50 last:border-0">
                            <div className={`w-7 h-7 rounded-lg ${bg} ${ic} flex items-center justify-center shrink-0 mt-0.5`}>
                                <Icon className="text-sm" />
                            </div>
                            <div>
                                <p className="text-[12px] font-medium text-slate-700 leading-relaxed">{text}</p>
