// app/(pages)/dashboard/treks/[id]/edit/EditTrekForm.tsx
'use client'
...
export default function EditTrekForm({ trek }: { trek: Trek }) {
    ...
    // Cover image
    const [coverPreview, setCoverPreview] = useState<string | null>(trek.cover_image)
    const [coverFile, setCoverFile] = useState<File | null>(null)
    const [removeCover, setRemoveCover] = useState(false)

    // Gallery — split into "existing kept" (URLs from DB) and "new" (fresh File uploads)
    const [existingGallery, setExistingGallery] = useState<string[]>(trek.gallery || [])
    ...
    const submitForm = (status: 'active' | 'inactive', formEl: HTMLFormElement) => {
        ...
        startTransition(async () => {
            try {
                await updateTrek(trek.id, formData)
                toast.success(status === 'active' ? 'Trek updated successfully!' : 'Trek saved as inactive!')
                setTimeout(() => router.push('/dashboard/treks'), 600)
            } catch (err: any) {
                toast.error(err.message || 'Failed to update trek')
    ...
    const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        submitForm(trek.status === 'active' ? 'active' : 'inactive', e.currentTarget)
    }

    const handlePublishClick = (e: React.MouseEvent<HTMLButtonElement>) => {
        const form = e.currentTarget.closest('form')
        if (form) submitForm('active', form)
    }

    const handleInactiveClick = (e: React.MouseEvent<HTMLButtonElement>) => {
        const form = e.currentTarget.closest('form')
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div className="ml-3">
                        ...
                        <h1 className="gloock text-xl md:text-2xl text-[#0d1f2d]">Edit Trek</h1>
                    </div>
                </div>
                <div className="hidden md:flex items-center gap-2">
                    ...
                    >
                        {isPending && pendingStatus === 'inactive' && <TbLoader2 className="animate-spin" />} Set inactive
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        {isPending && pendingStatus === 'active' && <TbLoader2 className="animate-spin" />}
                        {trek.status === 'active' ? 'Update trek' : 'Publish trek'}
                    </button>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5">
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-3">
                            ...
                            {existingGallery.map((src, index) => (
                                <div key={`existing-${index}`} className="relative rounded-lg overflow-hidden h-24">
                            ...
                            {newGalleryPreviews.map((src, index) => (
                                <div key={`new-${index}`} className="relative rounded-lg overflow-hidden h-24 ring-2 ring-[#D5E880]">
                            ...
                            <label className="flex flex-col items-center justify-center h-24 border-2 border-dashed border-slate-200 rounded-lg cursor-pointer hover:border-[#0A5482] hover:bg-blue-50/30 transition group">
                                ...
                                <input type="file" accept="image/*" multiple className="hidden" onChange={handleNewGalleryChange} />
                            </label>
                        </div>
                        <p className="text-[11px] text-slate-400">
                            Images with a green outline are new and not yet saved
                        </p>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5">
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <input
                                        name="max_altitude"
                                        type="number"
                                        defaultValue={trek.max_altitude ?? ''}
                                        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-600 outline-none bg-slate-50"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="difficulty" required defaultValue={trek.difficulty} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-600 outline-none bg-slate-50">
                            ...
                            <div>
                                ...
                                <select name="category" required defaultValue={trek.category} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-600 outline-none bg-slate-50">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <input
                                    name="best_season"
                                    type="text"
                                    defaultValue={trek.best_season?.join(', ') ?? ''}
